...
class SQL {
    ...
    insertIn(table, values, { options }) {
        if (options !== null || options !== "" || options !== undefined) {
            return `INSERT INTO ${table}(${options}) VALUES(${values});`;
        }
    }
    ...
    querywhere(table, { options, condition }) {
        return `SELECT ${options} FROM ${table} WHERE ${condition};`;
    }
    ...
    update(table, { options }) {
        return `UPDATE ${table} SET ${options};`;
    }
    ...
    updatewhere(table, { options, condition }) {
        return `UPDATE ${table} SET ${options} WHERE ${condition};`;
    }
     ...
     */
    alter(table, { options }) {
        return `ALTER TABLE ${table} ADD ${options}`;
    }
